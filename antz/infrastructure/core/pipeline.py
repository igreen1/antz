...
def run_pipeline(
    config: PipelineConfig, submit_pipeline: Callable[[PipelineConfig], None]
) -> Status:
...
def success(
    ...
    if next_config["curr_stage"] < len(next_config["stages"]):
        submit_pipeline(PipelineConfig.model_validate(next_config))
...
def restart(
    ...
    if (
        ...
        submit_pipeline(PipelineConfig.model_validate(new_config))
